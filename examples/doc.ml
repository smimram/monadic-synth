...
let () =
  let s =
    ...
    let q   = OSC.float "/oscControl/slider2" ~min:0.1 ~max:5. 1. in
    let f   = OSC.float ~mode:`Logarithmic "/oscControl/slider3" ~max:10000. 1500. in
    let a   = a   |> print "a" in
    let q   = q |> print "q" in
    let f   = f |> print "f" in
    osc
