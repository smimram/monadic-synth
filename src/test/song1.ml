...
let s =
  ...
  let melody = Instrument.play (note ~detune:false ~r:0.3 saw) (Pattern.midi tempo melody) in
  ...
  let bass = Instrument.play (note ~s:0.8 ~r:0.4 sine) (Pattern.midi tempo bass) in
  let kick = Pattern.repeat 16 [0.,0.25,`Note(69,1.8);0.,1.,`Nop] in
  let kick = Instrument.play_drum (fun ~on_die freq vol -> cmul vol (Note.Drum.kick ~on_die ())) (Pattern.midi tempo kick) in
  let snare = Pattern.repeat 16 [1.,0.25,`Note(69,0.8);0.,2.,`Nop] in
  let snare = Instrument.play_drum (fun ~on_die freq vol -> cmul vol (Note.Drum.snare ~on_die ())) (Pattern.midi tempo snare) in
  let s = synth in
