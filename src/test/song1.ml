...
let s ~dt =
  ...
  let melody = Instrument.play ~dt (note ~detune:false ~r:0.3 saw) (Pattern.midi tempo melody) in
  let melody = bind2 (Filter.first_order ~dt `Low_pass) (add (cst 600.) (cmul 300. (sine ~dt 10.))) melody in
  let melody = mul melody (OSC.float "/1/fader2" 1.) in
  let melody = bind3 (Filter.biquad ~dt `Low_pass) (OSC.float "/1/fader3" ~min:0.1 ~max:20. 0.5) (OSC.float "/1/fader4" ~max:10000. 10000.) melody in
  let melody = melody >>= Stereo.of_mono in
  let melody = melody >>= Stereo.dephase ~dt 0.01 in
  let vs = 0.7 in
  ...
  let bass = Instrument.play ~dt (note ~s:0.8 ~r:0.4 sine) (Pattern.midi tempo bass) in
  let kick = Pattern.repeat 16 [0.,0.25,`Note(69,1.8);0.,1.,`Nop] in
  let kick = Instrument.play_drum ~dt (fun ~on_die freq vol -> cmul vol (Note.Drum.kick ~dt ~on_die ())) (Pattern.midi tempo kick) in
