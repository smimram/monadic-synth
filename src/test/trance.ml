...
let s ~dt =
  ...
  let bass = Instrument.play ~dt bass_note (Pattern.midi tempo bass) in
  ...
  let bass = bass >>= Stereo.schroeder2 ~dt in
  let bass = let d = Note.duration tempo 0.5 in bass >>= Stereo.delay ~dt d ~feedback:0.1 ~ping_pong:d in
  ...
  let pd = Instrument.play_drums ~dt ~snare:(fun ~on_die freq vol -> cmul vol (Note.Drum.snare ~dt ~on_die ~lp:2400. ())) in
